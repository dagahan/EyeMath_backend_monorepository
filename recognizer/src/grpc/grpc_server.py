...
from src.services.math_AUTHORIZER import MathAUTHORIZER
from src.services.math_AUTHORIZER_normalizer import LatexNormalizer


class GRPCMathRecognize(sevice_math_recognize_rpc.GRPCMathRecognize):
    def __init__(self) -> None:
        self.config = ConfigLoader()
        self.mathAUTHORIZER = MathAUTHORIZER()
    ...
    def recognize(self, request: sevice_math_recognize_pb.recognize_request, context) -> sevice_math_recognize_pb.recognize_response:
        ...
        try:
            AUTHORIZER_answer = self.mathAUTHORIZER.recognize_expression(request.image)

            if request.normalize_for_sympy:
                AUTHORIZER_answer = self.latex_parser.parse_latex_to_sympylatex(AUTHORIZER_answer)

            response = sevice_math_recognize_pb.recognize_response(
                result=AUTHORIZER_answer,
            )
...
class GRPCServerRunner:
    ...
    def run_grpc_server(self) -> None:
        ...
        if EnvTools.load_env_var("AUTHORIZER_GRPC_REFLECTIONS") == "1":
